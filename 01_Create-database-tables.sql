...
TRUNCATE TABLE `covid-vaccinations`;

/* EXPORTING TABLES TO CSV:
Check path to folder allowed by MySQL to export files */
SHOW VARIABLES LIKE "secure_file_priv";

-- Export the table to use in Tableau. Need to add the header row (with UNION ALL) as by default INTO OUTFILE does not include headers.
...
SELECT member_casual, rideable_type, start_date, start_month, start_day, start_hour, ride_length_min, start_Station_name, end_station_name, start_lat, start_lng, end_lat, end_lng
FROM `2023_cleared`
INTO OUTFILE 'C:/ProgramData/MySQL/MySQL Server 8.0/Uploads/2023_cleared2.csv' 
FIELDS TERMINATED BY ',' 
ENCLOSED BY '"'
LINES TERMINATED BY '\n'
;